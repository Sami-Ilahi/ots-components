...
const TableBody = ({ tableBodyData, display }: Props) => {
  return (
    <tbody className="text-xs capitalize divide-y divide-gray-300 ">
      {tableBodyData?.map((item: dataTableItem, index: any) => (
        <tr key={index}>
          <td className="p-0.5">
          ...
          <td className="p-0.5">
            <div>
              <p>{item.FullySpecifiedName}</p>
            </div>
          </td>
          <td className="p-0.5">
            <div>
              <p>{item.DisplayNamePGOMedMij}</p>
            </div>
          </td>
        </tr>
      ))}
